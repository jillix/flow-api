...
EngineTools.prototype.modulePack = function (project, mod, callback) {
    ReadJson(
        this.packPath(
            Path.join(
                this.getProjectPath(project),
                "node_modules",
                mod
            )
        ),
 ...
 * @param {Function} callback The callback function.
 */
EngineTools.prototype.setNames = function (project, callback) {
    var self = this;
    self.getComposition(project, function (err, composition) {
        if (err) { return callback(err); }
        var arr = [];
...
EngineTools.prototype.getServicePath = function (app, mod) {
    var self = this;
    mod = Deffy(mod, "");
    return Path.join(self.getProjectPath(app), "node_modules", mod, ".service.json");
};
...
EngineTools.prototype.getModuleInfo = function (app, mod, callback) {
    ...
    if (!mod) {
        return self.appModules(self.getProjectPath(app), function (err, data) {
            if (err) { return callback(err); }
    ...
    SameTime([
      ...
      , self.modulePack.bind(self, app, mod)
        ...
        data[1] = Ul.merge(data[1], DEFAULT_PACKAGE);
        callback(null, {
            service: data[0]
          , package: data[1]
        });
