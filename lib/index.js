...
EngineTools.prototype.generateSymlinks = function (project, username, callback) {
    ...
    self.getInstances(project, function (err, instances) {
        if (err) { return callback(err); }
        SameTime(instances.filter(function (c) {
            return !FsPlus.isSymbolicLinkSync(c);
        }).map(function (c) {
            return ReadJson.bind(this, c);
        }), function (err, instances) {
            if (err) { return callback(err); }
            SameTime(instances.map(function (c) {
                return Ul.deepMerge(c, {
                    roles: {}
                  , client: {
                        config: {
                            template: {
...
EngineTools.prototype.checkNames = function (callback) {
    return function (link) {
        var inst = this;
        link.data(function (err, data) {
            if (err) { return link.end(err); }
