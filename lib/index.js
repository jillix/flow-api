    ...
    Fs.readFileAsync(path)
        ...
        .then(function (package) {
            ...
            methods.forEach(function (c) {
                if (c === "init" || !JxUtils.findFunction(mod, c)) { return; }
            ...
            Async.parallel(instances.map(function (c) {
                ...
                return function (opt_callback) {
                    var sPath = self.entrypoint(username, project, c.name, DEFAULT_HOST, c.roles["*"])
                      , target = Path.relative(CWD, compDir + "/" + c.name + ".json")
                      , sName = Path.relative(CWD, compDir + "/" + sPath)
                      ;

                    console.log(target, sName);
                    Fs.symlink(
                        target
                      , sName
                      , function (err) {
