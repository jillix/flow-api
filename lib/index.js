...
class FlowApi extends EngineBuilder {
    ...
    getModules (cb) {
        ...
        this.getAllInstances((err, data) => {
            if (err) { return cb(err); }
            iterateObject(data, c => {
                if (c.module.charAt(0) === "/") {
                    return;
                }
                appModules[c.module] = 1;
            });
            sameTime(bindy(Object.keys(appModules), this.getModulePackage.bind(this)), (err, data) => {
                if (err) { return cb(err); }
                cb(null, arrObj(data, "name"));
            });
        });
    }
