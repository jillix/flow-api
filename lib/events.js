...
/*
 *
 * returns sequence details in jsonld
 * @name getSequenceDetails
 *
 */
exports.getSequenceDetails = function (options, data, next) {
    var sequenceId = data.sequenceId;

    if (!sequenceId) {
        return next(new ApiError(data.res, 400, 'Missing sequence id.'));
    }
    ...
    jsonldClient.find(
        ...
        if (sequence.args && typeof sequence.args === 'string') {
            try {
                sequence.args = JSON.parse(sequence.args);
            } catch (e) {
                sequence.args = null;
            }
        }

        sequence['@context'] = 'http://schema.jillix.net/context/instance.jsonld';
