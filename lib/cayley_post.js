...
module.exports = (client, req, callback) => {
    ...
    let target;
    const triples = [];
    const save = (err, triples, remove) => {

        if (err) {
            return callback(err);
        }

        Network.remove(client, remove, (err, remove_result) => {

            if (err) {
                return callback(err);
            }

            Network.write(client, triples, (err, write_result) => {
            ...
            addTarget(client, triples, req.node, req.type, req.data, req.target, (err, target, triples) => {
                ...
                if (!req.target) {
                    addHashTriples(client, triples, target, req.name, req.json, false, (err, triples) => {
                        ...
                        Network.removeOut(client, req.node, [req.data.split(':')[0]], (err, remove) => {

                            if (err) {
                                return callback(err);
                            }

                            save(null, triples, remove);
                        });
                    });
                } else {
