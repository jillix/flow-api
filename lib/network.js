...
exports.removeNode = (client, node, callback) => {

    const remove = [];

    client.g.V(node).In(null, 'p').All((err, result) => {
        ...
        client.g.V(node).Out(null, 'p').All((err, result) => {
...
exports.removeOut = (client, node, edges, callback) => {
    ...
    // TODO delete nodes with no in edges (remove sub-tree)
    ...
    client.g.V(node).Out(edges, 'p').All((err, result) => {
