...
function writeTriple (writable, subject, predicate, object) {
    writable.write(subject + ' <' + predicate + '> ' + object + ' .\n');
}

function convertToRdf (writable, seq_id, jsonSeq) {
console.log("Convert to RDF:", seq_id);
    // ..convert to rdf
    jsonSeq[0].forEach((handler) => {
        // ..handler id
        // ..handler fn
        // ..handler state
        // ..handler args
    ...
    if (jsonSeq[1]) {
        jsonSeq[1].A  && writeTriple(writable, seq_id, "args", JSON.stringify(jsonSeq[1].A));
        // ..seq args
        // ..seq roles
        // ..seq error
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            const next = (file) => {
                fs.readFile(target + file, (err, data) => {
                    ...
                    try {
                        convertToRdf(rdf_file, file.split(".")[0], JSON.parse(data.toString()));
                    } catch (err) {
                        reject(err);
                    }
