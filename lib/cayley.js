...
exports.entrypoint = (g, entrypoint_name) => {
    return [
        g.V(entrypoint_name).In().In().Tag('subject')
        .Out('<http://schema.jillix.net/vocab/environment>', 'predicate')
        .Out('<http://www.w3.org/1999/02/22-rdf-syntax-ns#string>').All(),
        g.V(entrypoint_name).In().In().Tag('subject')
        .Out('<http://schema.jillix.net/vocab/sequence>', 'predicate').All()
    ];
};
